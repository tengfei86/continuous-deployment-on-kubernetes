...
public class MetadataUniqueConstraintTest extends BaseServiceTest {
    ...
    @Test
    public void testAddDropUniqueConstraintOnExistingBusinessObject() {
        ...
        // add unique constraint UWI and START TIME
        addUniqueConstraint(wellTestChildBoName);
        // drop unique constraint
        dropUniqueConstraint("UNIQUE_" + wellTestChildBoName + "_UWI_START_TIME");
    ...
    private void addUniqueConstraint(String boName) {
        try {
            // now make final request json
            String body = "{\n" +
                    ...
                    "}";
            // going to send request
            DSPDMResponse response = super.addUniqueConstraints(body);
            if (DSPDMConstants.Status.SUCCESS == response.getStatus()) {
                // print each message at appropriate log level
                response.getMessages().forEach(logger::log);
                logger.info("addCompositeUniqueConstraint passed.");
            } else {
                logger.info("addCompositeUniqueConstraint failed");
                throw response.getException();
            }
        } catch (Throwable e) {
            logger.error(e.getMessage(), e);
    ...
    private void dropUniqueConstraint(String constraintName) {
        try {
            // now make final request json
            String body = "{\n" +
                    "    \"BUS OBJ ATTR UNIQ CONSTRAINTS\": {\n" +
                    ...
                    "}";
            // going to send request
            DSPDMResponse response = super.dropUniqueConstraints(body);
            if (DSPDMConstants.Status.SUCCESS == response.getStatus()) {
                // print each message at appropriate log level
                response.getMessages().forEach(logger::log);
                logger.info("dropUniqueConstraints passed.");
            } else {
                logger.info("dropUniqueConstraints failed");
                throw response.getException();
            }
        } catch (Throwable e) {
            logger.error(e.getMessage(), e);
    ...
    public void testAddDropUniqueConstraintOnNewBusinessObject() {
        String wellTestChildBoName = "WELL TEST CHILD " + StringUtils.getNextRandomNumber(5);
        // introduce a new business object well test child. it has UWI and start time as its columns
        introduceNewBusinessObject_WellTestChild_With_UniqueConstraints(wellTestChildBoName);
    ...
    private void introduceNewBusinessObject_WellTestChild_With_UniqueConstraints(String boName) {
        try {
            // now make final request json
            String body = "{\n" +
                    "    \"BUSINESS OBJECT\": {\n" +
